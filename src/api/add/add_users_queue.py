...
df_update = pd.read_excel('./Files/AddUsersToQueue.xlsx')
...
for i, row in df_update.iterrows():
	...
	if queueId == '':
		...
		if user.empty:
			...
			request = conexiones_api.AddUserToQueue(queueId, userId)
			if request.status_code == 200:
				pprint("User {} added successfully to queue {}".format(user.iloc[0]['email'], queue.iloc[0]['name']))